...
//A filter method is used to loop through the drugs array and count the number of drugs that are Analgesics.
let analgesicDrugs =

drugs.filter((drug) => (drug.category === "Analgesic")
);

app.get("/drugs/count-analgesics", (req, res) => {
  const result = {count: analgesicDrugs.length};
  res.json(result);
});
